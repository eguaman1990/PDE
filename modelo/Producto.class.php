...
class Producto
{
    ...
    **/
	public function crear($idSubcategoria, $descripcion,$precioUnitario){
		try{
	...
	/**
		...
		* @param	$descripcion	Descripcion del Producto regitrado
		**/
	public function actualizar($id_producto, $id_subcategoria,$descripcion,$precio_unitario){
		try{
				...
				if($modo=="query"){
					while($rs=$res->fetch()){
						$campos[]=array(
							...
							"pro_precio_unitario"=>utf8_encode($rs["pro_precio_unitario"])
						);
					}
					return $campos;
				}else{
