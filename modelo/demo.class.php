...
class Porcion
{
	...
	public function setIdInventario($idSubcategoria){
	  if(strlen(trim($idSubcategoria))!=0){
		  ...
		  $error=array(
				...
				'admin'=>'atributo id_categoria,function setIdInventario '.$this->__toString()
		  );
		  $this->myException->addError($error);			  
	  }//fin del if
	}//fin del setIdPorcion
    ...
    **/
	public function crear($idSubcategoria, $id_producto,$porcion,$unidad){
		try{
			$this->setIdInventario($idSubcategoria);
			...
			if ($this->myException->getEstado()==0){
					...
					$parametros=array(
						"ID_SUBCATEGORIA"=>$this->getIdInventario(),
						"PRO_DESCRIPCION"=>$this->getIdProducto(),
						"PRO_PRECIO_UNITARIO"=>$this->getPorcion()
					);
					$rs1=$this->bd->insert('PorcionS',$parametros);
	...
	public function actualizar($id_porcion, $id_inventario,$id_producto,$porcion,$unidad){
		try{
			...
			if ($this->myException->getEstado()==0){
					...
					$parametros=array("PRO_DESCRIPCION"=>$this->getIdProducto(),"ID_SUBCATEGORIA">$this->getIdInventario(),
					"PRO_PRECIO_UNITARIO"=>$this->getPorcion());
    ...
    */
	public function listar($sqlend,$sqlGroupBy="",$sqlOrden="",$sqlLimit="",$modo="query"){
		try{
			...
			$nro_registros=0;
			if($modo=="query"){
				$sqlCampos=" p.id_porcion,p.id_inventario,p.pro_idProducto,p.pro_porcion,s.subca_idProducto ";	
			}else{
				$sqlCampos=" count(*) as hay ";
			}
			//CONCATENO SQL CAMPOS Y SQLEND
			$sql="SELECT ".$sqlCampos." FROM productos p  INNER JOIN subcategorias s ON p.id_inventario = s.id_inventario
				...
				if($modo=="query"){
					while($rs=$res->fetch()){
						$campos[]=array(
							...
							"subca_idProducto"=>utf8_encode($rs["subca_idProducto"]),
							"pro_idProducto"=>utf8_encode($rs["pro_idProducto"]),
							"pro_porcion"=>utf8_encode($rs["pro_porcion"])
						);
					}
					return $campos;
				}else{
