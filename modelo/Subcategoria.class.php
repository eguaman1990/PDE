...
//
// Clase Subcategoria que me va a permitir listar, actualizar,
// buscar,eliminar,crear Subcategorias; ademas permite tener un 
// contSubcategoria de acesso del sistema permitiendo agregar y quitar permisos
// 
...
class Subcategoria
{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	#####################################	METODOS GET	#######################################################
	public function getIdSubcategoria(){ return $this->_idSubcategoria;	}
	...
	public function setIdSubcategoria($idSubcategoria){
	  if(strlen(trim($idSubcategoria))!=0){
		  ...
		  $error=array(
				'user'=>'CAMPO idSubcategoria ESTA VACIO',
				'admin'=>'atributo idSubcategoria,function setIdSubcategoria '.$this->__toString()
		  );
		  $this->myException->addError($error);			  
	  }//fin del if
	}//fin del setIdSubcategoria
	...
	/**
		...
		*	Me permite Crear un Subcategoria al Sistema
		* @return RETORNA VALOR 1 SI ES INGRESADO EXITOSAMENTE
		* @param  	$id_categoria valor con el que el Subcategoria se loguea
		* @param	$descripcion	Descripcion del Subcategoria regitrado
    **/
	public function crear($idCategoria, $descripcion){
		try{
			...
			if ($this->myException->getEstado()==0){
					#ES 0 SI NO EXISTE EL Subcategoria					
					$parametros=array(
						"ID_CATEGORIA"=>$this->getIdCategoria(),
						"SUBCA_DESCRIPCION"=>$this->getDescripcion()
					);
					$rs1=$this->bd->insert('SubcategoriaS',$parametros);
	...
	/**
		...
		*	Me permite Actualizar un Subcategoria existente en el Sistema
		* @return RETORNA VALOR 1 SI ES INGRESADO EXITOSAMENTE
		* @param  $id_categoria valor con el que el Subcategoria se loguea
		* @param	$descripcion	Descripcion del Subcategoria regitrado
		**/
	public function actualizar($id_subcategoria, $id_categoria,$descripcion){
		try{
			...
			/***	aki pregunto si se cumplieron todas las validaciones de los SET	***/
			if ($this->myException->getEstado()==0){
					$condicion=array("id_subcategoria"=>$this->getIdSubcategoria());
					$parametros=array("SUBCA_DESCRIPCION"=>$this->getDescripcion(),"ID_CATEGORIA">$this->getIdCategoria());
					
					$rs1=$this->bd->update('Subcategorias',$parametros,$condicion);
					if ($this->bd->myException->getEstado()==0){
						return 1;
					}else{
						$this->myException->setEstado(1);
	...
	/**
		* Funcion Buscar que me permite validar si el Subcategoria existe
		* @return RETORNA VALOR 1 si el Subcategoria existe, VALOR 0 si el Subcategoria no existe
		* @param  $idSubcategoria valor con el que el Subcategoria se loguea
     */
	public function buscar($idSubcategoria){
		$this->setIdSubcategoria($idSubcategoria);
		try{
			if ($this->myException->getEstado()==0){
				#verifico que no existan ese Subcategoria
				$strsql="select id_subcategoria from Subcategorias where id_subcategoria=?";
				$condicion=array($this->getIdSubcategoria());
				...
				if ($this->bd->myException->getEstado()==0){
					...
					return 0;
				}//fin del if que me contSubcategoriaa que no se haya caido la consulta
			}else{
				return 0;
			}//fin del if que me contSubcategoriaa que no se haya caido l coneccion
		}catch(Exception $e){
			$this->myException->setEstado(1);
			$error=array(
			   'user'=>'SE PRODUJO UN ERROR. COMUNICARSE CON EL ADMINISTRADOR DEL SISTEMA.',
    ...
    */
	public function listar($sqlend,$sqlGroupBy="",$sqlOrden="",$sqlLimit="",$modo="query"){
      ...
      $campos=array();
			if($modo=="query"){
				$sqlCampos=" s.id_categoria,s.id_subcategoria,s.subca_descripcion,c.ca_descripcion ";	
			}else{
				$sqlCampos=" count(*) as hay ";
			}
			//CONCATENO SQL CAMPOS Y SQLEND
			$sql="SELECT ".$sqlCampos." FROM subcategorias s INNER JOIN categorias c ON s.ID_CATEGORIA=c.ID_CATEGORIA
			 WHERE 1=1 ".$sqlend;
			/*echo $sql;
      exit();*/
				...
				if($modo=="query"){
					while($rs=$res->fetch()){
						$campos[]=array(
							...
							"ca_descripcion"=>utf8_encode($rs["ca_descripcion"]),
